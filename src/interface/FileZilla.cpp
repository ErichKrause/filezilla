...
// Part of FileZilla, which is Copyright Tim Kosse and distributed under
// the GNU General Public License.

// 3 September 2009: Laurence Field: Modified to find its resources within
// an Application Bundle structure, rather than a global installation.
...
static const wxString& get_bundle_data_dir()
{
	static wxString dir;
...
wxString CFileZillaApp::GetDataDir(wxString fileToFind) const
{
	...
	return _T("");
#elif 1
        const wxString &path = get_bundle_data_dir();
	if (FileExists(path + fileToFind))
		return path;
...
#elif 1
	wxString executable =
		GetDataDir(_T("/../Helper Programs/fzsftp"))
		+ _T("/../Helper Programs/fzsftp");

	if (!wxFileName::FileExists(executable))
	{
		wxMessageBox(wxString::Format(_("%s could not be found. Without this component of FileZilla, SFTP will not work.\n\nPlease download FileZilla again. If this problem persists, please submit a bug report."), "fzsftp"),
			_("File not found"), wxICON_ERROR);
		executable.clear();
	}
#else
